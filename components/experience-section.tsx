  ...
  return (
    <section id="experience" className="relative py-24 px-4 sm:px-6 lg:px-8 overflow-hidden">
      <div className="max-w-6xl mx-auto">
        ...
        >
          <h2 className="text-4xl md:text-5xl font-bold text-center mb-16">Work Experience</h2>
          ...
          <div className="relative">
            ...
            <div className="space-y-12">
              {experiences.map((exp, index) => (
                ...
                >
                  {/* Left Side */}
                  <div className={`w-full md:w-[calc(50%-2rem)] pl-12 md:pl-0 mb-8 md:mb-0 ${index % 2 !== 0 ? "order-1" : "order-1 md:order-3"}`}>
                     {index % 2 !== 0 && (
                       <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-gray-200 dark:border-slate-700 shadow-lg hover:shadow-xl transition-shadow group text-left md:text-right">
                        <div className="flex items-center md:justify-end gap-2 text-blue-600 dark:text-blue-400 mb-2">
                          <span className="font-semibold hidden md:inline">{exp.company}</span>
                          <Briefcase size={16} />
                          <span className="font-semibold md:hidden">{exp.company}</span>
                        </div>
                        
                        <h3 className="text-xl font-bold mb-2 text-gray-900 dark:text-white">{exp.role}</h3>
                        ...
                        <div className="flex flex-wrap md:justify-end gap-2">
                          {exp.tags.map(tag => (
                            <span key={tag} className="px-3 py-1 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-300 text-xs rounded-full font-medium">
                              {tag}
                            </span>
                          ))}
                        </div>
                  ...
                  {/* Right Side */}
                  <div className={`w-full md:w-[calc(50%-2rem)] pl-12 md:pl-0 ${index % 2 === 0 ? "order-1 md:order-3" : "order-1"}`}>
                    {index % 2 === 0 && (
